...
public:
  Leaf_kv() {}
  Leaf_kv(const GlobalAddress& rev_ptr,const int& leaf_type,const int& keylen,const int& valen,const Key& key, const Value& value) : rev_ptr(rev_ptr), f_padding(0), leaf_type(leaf_type), valid(1), key(key), value(value),  lock_byte(0) { set_consistent(); }
...
public:
  ...
  InternalEntry(uint8_t partial, const BufferEntry& e) :
                partial(partial),child_type(e.node_type),empty(0),node_type(e.leaf_type),packed_addr(e.packed_addr) {}
