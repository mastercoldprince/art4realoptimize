...
public:
  ...
  InternalEntry(uint8_t partial, uint8_t child_type,const GlobalAddress &addr) :
                partial(partial), child_type(child_type), empty(3),node_type(0),packed_addr{addr.nodeID, addr.offset >> ALLOC_ALLIGN_BIT} {}
  InternalEntry(uint8_t partial, uint8_t child_type,NodeType node_type, const GlobalAddress &addr) :
                partial(partial),child_type(child_type), empty(2), node_type(static_cast<uint8_t>(node_type)),  packed_addr{addr.nodeID, addr.offset >> ALLOC_ALLIGN_BIT} {}
  ...
  InternalEntry(uint8_t partial, const InternalEntry& e) :
                partial(partial), child_type(e.child_type), empty(4), node_type(e.node_type),packed_addr(e.packed_addr) {}
  InternalEntry(uint8_t partial, const BufferEntry& e) :
                partial(partial),child_type(e.node_type),empty(5),node_type(e.leaf_type),packed_addr(e.packed_addr) {}
